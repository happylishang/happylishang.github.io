...
先看关键点1：其实就是确定当前低内存对应的阈值；关键点2 ：找到比该阈值优先级低，并且内存占用较多的进程（tasksize = get_mm_rss(p->mm)其实就是获取内存占用），将其杀死。如何杀死的呢？很直接，通过Linux的中的信号量，发送SIGKILL信号直接将进程杀死。到这就分析完了LomemoryKiller内核部分如何工作的。其实很简单，**一句话：被动扫描，找到低优先级的进程，杀死。**
