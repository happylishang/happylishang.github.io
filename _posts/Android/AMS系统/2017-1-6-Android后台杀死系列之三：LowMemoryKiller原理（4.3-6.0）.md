...
本篇是Android后台杀死系列的第三篇，前面两篇已经对杀死注意事项，杀死恢复机制做了分析，所以本篇主要讲解的是Android后台杀死原理：LowMemoryKiller。LowMemoryKiller是Andorid基于Linux的oomKiller原理所扩展的一个多层次oomKiller，属于内核模块，运行在内核空间，在详细分析它的原理之前不妨自己想一下，假设让你设计一个LowMemoryKiller，你会如何做，这样一个系统需要什么功能模块呢？
...
以上几个问题便是一个MemoryKiller模块需要的基本功能。Android系统以Linux内核为基础，所以对于进程的管理自然离不开Linux本身提供的机制。例如：
