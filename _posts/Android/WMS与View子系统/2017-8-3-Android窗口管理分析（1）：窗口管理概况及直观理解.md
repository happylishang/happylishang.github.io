---
layout: post
title: Android窗口管理分析（1）：窗口管理概况及主观理解
category: Android
...
每一个图层分配一个画布，画布是进程间共享的，WMS控制着画布的添加与次序，SF负责画布的图层的混合，每个APP负责每个图层的绘制，图层之间数据的共享是通过匿名内存来实现的。

其实就是MAP+tmpfs文件系统，你可以理解成SF为APP申请一块内存，然后通过binder将这块内存相关的信息传递APP端，APP端往这块内存中绘制内容，绘制完毕，同时SF图层混排，再有SF将数据渲染到屏幕。
