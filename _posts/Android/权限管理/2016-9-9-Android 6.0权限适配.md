...
6.0之前Android的权限都是在安装的时候授予的，6.0之后，需要动态申请权限App开发时，如果将targetSdkVersion设置为23，当运行在Android 6.0 +的手机上时，对于一些敏感权限的获取就会采用动态请求的方式:

targetSDKVersion：简单来说就代表着你的App能够适配的系统版本，意味着你的App在这个版本的手机上做了充分的 前向 兼容性处理和实际测试。其实我们写代码时都是经常干这么一件事，就是 if(Build.VERSION.SDK_INT >= 23) { ... } ，这就是兼容性处理最典型的一个例子。如果你的target设置得越高，其实调用系统提供的API时，所得到的处理也是不一样的，甚至有些新的API是只有新的系统才有的，
...
#请求权限：实时检查，没有就请求，因为随时可以在设置里面把权限给更新


需要权限的时候去显示的请求，授权的对话框不可定制，可以添加一些解释。无论哪种权限，都需要在Manifest列出来，所不同的是，Android6.0+版本的手机会请求用户的允许，同时，这也是对老版本的一种兼容。
