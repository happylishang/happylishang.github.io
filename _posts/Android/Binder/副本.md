...
如果是BC_REPLY,一定是唤醒之前发送等待的线程，这个是100%的，如果是BC_TRANSACTION，那就不一定了，当进程P1的线程T1向进程P2发送请求时，驱动会先查看一下线程T1是否也正在处理来自P2某个线程请求但尚未完成（没有发送回复）。这种情况 通常发生在两个进程都有Binder实体并互相对发时请求时。假如驱动在进程P2中发现了这样的线程，比如说T2，就会要求T2来处理T1的这次请求。因为T2既然向T1发送了请求尚未得到返回包，说明T2肯定（或将会）阻塞在读取返回包的状态。这时候可以让T2顺便做点事情，总比等在那里闲着好。而且如果T2不是线程池中的线程还可以为线程池分担部分工作，减少线程池使用率。*/
			 
比如在startActivity的时候，APP主线程会向SystemServer进程进行binder通信，请求AMS服务，而在AMS startActivity的时候，会通过ApplicationThread的BinderProxy向APP端发送Binder请求，去让APP端新建Activity对象，并调用onCreate等回调函数，并且，在完成后，还要APP端向SystemServer端发送确认启动的信息，告知AMS服务去更新数据，那么这里就有一个循环：

* 进程P1的线程T1请求P2进程的A服务，	T1睡眠
* P2中的Binder线程Binder_P2-1在执行A服务的时候，需要请求P1端的B服务 Binder_P2-1睡眠
* P1端的Binder线程Binder_P1-1在执行B服务的时候，需要P2端的C服务,Binder_P1-1睡眠，Binder_P2-1唤醒
* B服务执行完毕，唤醒Binder_P1-1，Binder_P2-1睡眠在当前线程的等待
