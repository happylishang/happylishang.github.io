...
IServiceConnection conn = IServiceConnection.Stub.asInterface(b)，其实就是将传输的InnerConnection读取出来，不过由于Binder驱动将BINDER_TYPE_BINDER转换成了BINDER_TYPE_HANDLE，这里其实会实例化一个IServiceConnection.Proxy代理对象，因为在Parcel  readStrongBinder的时候，会做一个关键的处理:
...
再看关键点2，如果是BpBinder，则需要首先在gBinderProxyOffsets中查找，是不是已经新建了Java层代理BinderProxy对象，如果新建就不需要重复新建了，则需要重新绑定，？？？？
