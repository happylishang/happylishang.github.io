...
在LoadApk中IServiceConnection对象是通过context键值来存储ServiceDispatcher对象，而ServiceDispatcher对象内存会有个InnerConnection对象，该对象就是getServiceDispatcher的返回对象，Binder.java中的Binder其实是对native BBinder的一个java层封装,Java层Binder的实例化，其实是通过JNI到native层实例化一个BBinder对象，
...
的时候，sd其是就是一个InnerConnection，这是个Binder实体，其主要目的是向AMS传递一个Binder句柄，方便AMS通知APP端，这里可以跟到AMS看一下
...
Parcel的writeStrongBinder会将Binder实体写入到Parcel中去，这里首先看一下 Parcel data = Parcel.obtain();在java层Parcel只是一个容器，具体Parcel相关的操作都在Native层
...
flat_binder_object经过一次拷贝写入目标进程，并执行BINDER_TYPE_BINDER与BINDER_TYPE_HANDLE的转换，如下：
...
首先看一下关键点1 ，这里将内核数据映射到一个用户空间的Parcel对象中去，之后在调用目标Service的transact函数，进而调用他的onTrasanct函数 ,对于bindService，就是调用AMS native Binder的onTrasanct，
...
在AMS端，通过Binder驱动的转换，其实会将InnerConnection转成IServiceConnection.Proxy代理对象，
