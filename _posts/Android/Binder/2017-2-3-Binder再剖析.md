...
Binder在Android中的重要程度不言而喻，可以看做整个Android的血管系统，负责不同模块进程间的通信。对Binder的理解上，说大可大，说小可小，说大了，有binder驱动、基于binder扩展到一些通信框架、binder的一些讣告、一些一次拷贝之类的原理，说小，其实就是一句话：Binder负责进程间通信，将进程A的数据，发送到进程B，可以说Binder就负责数据传输。只不过Android在Binder原始的基础上扩展了一些，比如Servicemanager，transact框架，整体抓起来还是比较吃力，现在，我们从问题深入原理，会比较清楚整个框架不妨看看如下问题，你能深入多少：


*  Binder传输数据的大小限制（内核这个呢4M 上层限制1m-8k），传输Bitmap过大，就会崩溃的原因
*  Binder一次拷贝原理
*  Binder线程唤醒与进程唤醒
*  Binder不同层次使用的数据结构
*  系统service跟AIDL service的区别
*  自定义Service通过Addservice限制
*  bindservice的流程
*  binder通信双通道的理解
*  Binder实体与Handler的转换
*  binder驱动中数据缓存的释放
...
*  Binder的阻塞与非阻塞？
*  用户空间与内核空间map关系
*  BinderProxy的实例化（通过Parcel的read）
*  Parcel read与write Binder的原理
*  Binder线程 主线程 Binder主线程 Binder普通线程
*  Binder线程的动态增加
*  Zygote fork的进程为何天然支持binder通信
*  不同版本Servicemanager主线程的功能不同，4.3Binder线程，6.0非binder线程
*  同一个binder_trasaction位于两个栈上
*  binder_work私有唤起队列
*  binder中数据结构的传递，与数据的传递的区分
*  C/S中阻塞与非阻塞的原理，waitforresponse（null，null） 直接等到COMPLETE就完工，不会继续等待，这也是为什么说Binder通信是客户端同步二服务端是异步，客户端发完请求后要等待返回结果，但是服务端处理完，发送结果后，不会等Client反馈
*  
