...
# 举个栗子：请求定位服务LocationManagerService: CM12源码（4.3）
...
Android6.0开始，原生支持runtime-permission机制，让用户在任何时候都可以取消/授权，并且能够在请求服务之前知晓是否已经获得所需要的权限。是的APP端能够根据需求，自主控制权限的申请，更加灵活。首先先看一下权限的查询：support-v4兼容包里面提供了一个工具类PermissionChecker，可以用来检查权限获取情况。
