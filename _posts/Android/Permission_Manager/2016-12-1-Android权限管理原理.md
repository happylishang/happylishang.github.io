...
对于国产ROM，其实还有个不太好理解的地方，那就是鉴权的时候，采用的是同步Binder请求的方式，当申请权限的对话框弹出的时候，请求服务的线程会暂停，一直等到授权结果返回，这里不太好理解，为什么系统的对话框都显示出来还是同步呢，这里的表现命名就是异步啊，客户端如何知道服务端的鉴权对话框有点击选择处理呢？其实这里牵扯到Binder通信的问题，Binder可以说是采用了对于客户端同步，服务端异步的一个机制，在这里我们只看下客户端请求的时候，是如何处理的？接着上面的对话框弹出，里面有个一PermissionDialogResult，很奇葩，
	...
	// PermissionDialogResult里面有个阻塞，如果你在主线程调用，就会阻塞
...
由于上面的noteOperation在调用askOperationLocked后，就会通过return userDialogResult.get();get()函数会通过wait阻塞在那里， 一直到有操作或者超时，这个超时一般是小于ANR的时间，所以不用担心ANR，直到通过notifyAll通知操作结束。这里客户端也会阻塞，等待返回数据。
...
// 这里如何保证阻塞回调的呢？
  ...
  我们知道，在Binder请求通信的时候，就算是void，不需要返回的函数，还是会有个reply.writeNoException();通知客户端执行成功，而客户端也会一直通过 transact等待，等待有数据返回，或者只告诉客户端执行成功。这是阻塞的根本。
