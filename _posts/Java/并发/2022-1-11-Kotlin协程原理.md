...
###  suspend与唤起与线程睡眠[协程Task]  

**suspend与线程睡眠没有任何关系，协程的挂起并不一定导致线程的休眠，对于当前协程所处的线程，除了delay，大部分时候其实都是被会被处理成回调**，只要当前协程上下文没有亲自处理耗时阻塞任务，就不会导致对应线程挂起，挂起线程大部分不是协程导致的。
...
其实直观上也好理解，耗时任务在哪个线程执行，会阻塞哪个线程，其他线程也没必要强等待，靠隐性实现的回调就可。
